...
export default function NavBarButtons({ session }: { session: Session | null }) {
  ...
  return (
    <>
            ...
            <DropdownMenu.Item className="flex cursor-pointer items-center border-b border-slate-100 px-5 py-3 text-sm font-medium text-sky-700 outline-none hover:bg-sky-100 hover:text-sky-700">
              ...
              >
                ...
                <span className="ml-0 pt-0.5 text-[11px] font-normal leading-snug text-slate-500">
                  What is MemicOS? How do I use it?
                </span>
              </Link>
            </DropdownMenu.Item>
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger className="hidden cursor-pointer items-center whitespace-nowrap rounded-full px-2.5 py-1 text-[13px] transition-all hover:bg-sky-100 hover:text-sky-700 focus:outline-none data-[state=open]:bg-sky-700 data-[state=open]:text-white ">
          Evals
          <ChevronDown className="ml-0.5 h-3.5 w-3.5" />
        </DropdownMenu.Trigger>
        <DropdownMenu.Portal>
          <DropdownMenu.Content className="z-50 min-w-[8rem] max-w-[20rem] overflow-hidden rounded-md bg-white p-1 shadow-[0px_10px_38px_-10px_rgba(22,_23,_24,_0.35),_0px_10px_20px_-15px_rgba(22,_23,_24,_0.2)]">
            <DropdownMenu.Item className="flex cursor-default items-center px-3 py-2 text-sm font-medium text-slate-500 outline-none">
              <span className="flex flex-row items-center gap-x-2 text-xs text-slate-400">
                <span className="rounded bg-amber-400 px-1.5 py-0.5 text-[11px] font-semibold text-amber-800">
                  BETA
                </span>
                Evals are under active development and may be inaccurate or incomplete.
              </span>
            </DropdownMenu.Item>
            <DropdownMenu.Item className="flex cursor-pointer items-center px-3 py-2 text-sm font-medium text-sky-700 outline-none hover:bg-sky-100 hover:text-sky-700">
              <Link
                href="/eval/auto-interp"
                prefetch={false}
                className="flex w-full flex-col items-start justify-center text-sm"
              >
                Auto-Interp
                <span className="ml-0 pt-1 text-xs text-slate-500">
                  Comparing the performance between three models for explanation using the token-activation pairs.
                  Scored with the detection method.
                </span>
              </Link>
            </DropdownMenu.Item>
            {/* <DropdownMenu.Item className="flex cursor-pointer items-center px-3 py-2 text-sm font-medium text-sky-700 outline-none hover:bg-sky-100 hover:text-sky-700">
              <Link
                href="/eval/sae-simple"
                prefetch={false}
      ...
      <Link
        href="mailto:johnny@memicos.org?subject=MemicOS"
        className="flex cursor-pointer items-center whitespace-nowrap px-0 py-0.5 text-[13px] transition-all hover:text-sky-700 hover:underline sm:mb-0 sm:hidden sm:px-1 sm:py-0 "
      >
        Contact
      </Link>
