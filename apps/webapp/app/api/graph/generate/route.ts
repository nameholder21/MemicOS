...
const SCAN_TO_SOURCE_URLS = {
  'gemma-2-2b': [
    'https://memicos.org/gemma-2-2b/gemmascope-transcoder-16k',
 ...
 *     description: Creates a new graph by analyzing the provided text prompt using the specified model. You'll get a link to access the graph visualization directly on MemicOS's UI, and the graph will be saved to S3 and metadata stored in the database.
 *     tags:
 *       - Attribution Graphs
 *     requestBody:
 *       required: true
...
export const POST = withOptionalUser(async (request: RequestOptionalUser) => {
  try {
    ...
    graph.metadata = {
      ...graph.metadata,
      info: {
        creator_name: `${request.user?.name || 'Anonymous'} via MemicOS`,
        creator_url: 'https://memicos.org',
