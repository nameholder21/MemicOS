...
* You can compare 200+ SAEs of varying sparsity, dictionary size, architecture, and training time on [MemicOS](https://memicos.org/sae-bench).   
...
<a href="https://www.memicos.org/sae-bench" target="_blank" rel="noopener noreferrer" className="ignore-saebench gap-x-1.5 flex flex-row items-center rounded-full bg-sky-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-sky-700 sm:ml-1 sm:flex">
...
*Credit: Callum McDougall, [ARENA Tutorials](https://colab.research.google.com/drive/1ePkM8oBHIEZ2kcqAiA3waeAmz8RSdHmq#scrollTo=6lix5OebZ38q)*
...
To quantify feature absorption, we follow the example in [Chanin et al.](https://arxiv.org/pdf/2409.14507v3) and use a first letter classification task. First, tokens consisting of only English letters and an optional leading space are split into a train and test set, and a supervised logistic regression probe is trained on the train set using residual stream activations from the model. This probe is used as ground truth for the feature direction in the model. Next, k-sparse probing is performed on SAE latents from the train set to find which latents are most relevant for the task. The k=1 sparse probing latent is considered as a main SAE latent for the first letter task. To account for feature splitting, as k is increased from k=n to k=n+1, if the F1 score for the k=n+1 sparse probe represents an increase of more than τ_{fs} than the F1 of the k=n probe, the k=n+1 feature is considered a feature split and is added to the set of main SAE latents performing the first letter task. We use τ_fs=0.03 in line with [Chanin et al](https://arxiv.org/pdf/2409.14507v3).
...
Follow-up work by [Chaudhary & Geiger](https://github.com/MaheepChaudhary/SAE-Ravel) adapted the RAVEL benchmark to sparse autoencoders. While our implementation is closely related to the original RAVEL evaluation, a differential binary masking-based latent selection in line with [Chaudhary & Geiger](https://github.com/MaheepChaudhary/SAE-Ravel) is forthcoming. RAVEL is not featured on MemicOS, yet.
</details>
...
The MemicOS interface contains benchmark results for Gemma-Scope SAEs (JumpReLU architecture, 16k - 1M width, on Gemma-2-2B and -9B models) and the open-source SAEBench suite of sparse autoencoders (Standard (ReLU) and TopK architecture, 4k, 16k, 65k width for Gemma-2-2B). 
...
*Absorption score across architectures for fixed 65k width. Note that unlike our other metrics, a lower score or rate of feature absorption is better.*
...
*Diverse metric scores require tradeoffs in sparsity.*
...
*The optimum sparsity for sparse probing significantly varies between layers.*
...
*The automated interpretability score monotonically increases throughout training on average. Notably, TopK is more token efficient than the Standard SAE architecture.*
...
*SAEs in the high sparsity regime (low L0) perform relatively better on automated interpretability but worse on reconstruction error (Loss Recovered).*
...
*The optimal sparsity differs for SCR and TPP – two metrics that are conceptually closely related.*
