...
export const sendEmail = async (
  ...
  if (awsSesClient) {
    const command = new SendEmailCommand({
      FromEmailAddress: `MemicOS <${CONTACT_EMAIL_ADDRESS}>`,
      FeedbackForwardingEmailAddress: CONTACT_EMAIL_ADDRESS,
      ConfigurationSetName: 'MemicOS',
      Destination: {
    ...
    const result = await resendClient.emails.send({
      from: `MemicOS <${CONTACT_EMAIL_ADDRESS}>`,
      to: emailAddress,
      subject,
      html,
...
export const sendLoginEmail = async (emailAddress: string, signInURL: string) => {
  ...
  const subject = `Confirm Sign In to MemicOS`;
...
export const sendWelcomeEmail = async (emailAddress: string, unsubscribeCode: string) => {
  const subject = 'Welcome to MemicOS';
