  ...
  return async (request: RequestAuthedUser, response: NextResponse) => {
    ...
    if (!authenticatedUser) {
      return NextResponse.json(
        {
          error:
            'This endpoint requires authorization. Specify your API key in the header x-api-key. Your API key is under Settings on memicos.org.',
        },
        { status: 401 },
      );
    }
  ...
  return async (request: RequestAuthedAdminUser, response: NextResponse) => {
    ...
    if (!authenticatedAdminUser) {
      return NextResponse.json(
        {
          error:
            'This endpoint requires authorization and admin access. Specify your API key in the header x-api-key. Your API key is under Settings on memicos.org.',
        },
        { status: 401 },
      );
    }
