...
def parse_env_and_args():
    ...
    args.device = os.getenv("DEVICE", "cpu")
...
def list_available_options():
    ...
    df = df[df["memicos_id"].notna()]  # Remove rows with None memicos_id
    models = df["model"].unique()
    df = df.sort_values(by=["model", "memicos_set"])

    print("Available models and SAE sets:")
    for model in models:
        print(f"  {model}:")
        model_df = df[df["model"] == model]
        sae_sets = model_df["memicos_set"].unique()
        for sae_set in sae_sets:
            set_size = len(model_df[model_df["memicos_set"] == sae_set])
