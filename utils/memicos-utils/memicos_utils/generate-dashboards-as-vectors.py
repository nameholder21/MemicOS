# This generates dashboards for your SAE or set of vectors for a model, for direct import into MemicOS.
# Once the outputs are generated, they can be uploaded to the memicos-datasets S3 bucket, where anyone can import and experiment with them.
#
...
#     --memicos-source-set-id=axbench-reft-r1-res-16k \
#     --memicos-source-set-description='Residual Stream - 16k' \
...
from memicos_utils.db_models.source_release import SourceRelease
from memicos_utils.db_models.model import Model
from memicos_utils.db_models.source_set import SourceSet
from memicos_utils.db_models.source import Source
from memicos_utils.db_models.activation import Activation
from memicos_utils.db_models.feature import Feature
import dotenv
from enum import Enum
from huggingface_hub import hf_hub_download
import torch
from sae_dashboard.memicos.vector_set import VectorSet
from sae_dashboard.memicos.memicos_vector_runner import (
    MemicOSVectorRunner,
    MemicOSVectorRunnerConfig,
)
...
OUTPUT_DIR = "./exports"
# directory used for intermediate outputs (from saedashboard, before conversion to memicos format)
...
def main(
    ...
    release_id: Annotated[
        ...
        make_option(
            "--release-id",
            help_text="[Release] Release ID: Enter the release id (eg gemma-scope). Must be alphanumeric, but can also include dashes. Your release will be available at https://[memicos_domain]/[release_id]",
        ),
    ],
    ...
    memicos_source_set_id: Annotated[
        str,
        make_option(
            "--memicos-source-set-id",
            help_text="[Source] MemicOS Source Set ID: All dashboards on MemicOS belong to a 'Source Set', which is an identifier. Please specify what the 'source set id' is for your data - it should be short and descriptive of the author, hook, and optionally, width (number of features/vectors per layer).\nFor example, an example source set ID is gemmascope-res-16k. The URL is then https://[memicos_domain]/gemma-2-2b/gemmascope-res-16k.\nDo not include the layer number - that will automatically be prepended later.",
        ),
    ],
    memicos_source_set_description: Annotated[
        str,
        make_option(
            "--memicos-source-set-description",
            help_text="[Source] MemicOS Source Set Description: When this source set is displayed on MemicOS, this is the description that will be shown. Usually, it is a short human-readable hook and width for this source. Eg Residual Stream - 16k",
        ),
    ],
    ...
    try:
        ...
        intermediate_output_dir = (
            f"{INTERMEDIATE_OUTPUT_DIR}/{model_name}/{memicos_source_set_id}"
        )
        ...
        runner_config = MemicOSVectorRunnerConfig(
        ...
        runner = MemicOSVectorRunner(vector_set, runner_config)
        runner.run()

        print("Activations generated successfully.")

        print("Converting to MemicOS format for final output...")
        ...
        for file in sorted(
            ...
            with open(sourceset_file_path, "w") as f:
                sourceset = SourceSet(
                    modelId=model_name,
                    name=memicos_source_set_id,
                    creatorId=DEFAULT_CREATOR_ID,
                    createdAt=created_at,
                    creatorName=creator_name,
                    releaseName=release_id,
                    description=memicos_source_set_description,
                    visibility="PUBLIC",
                )
            ...
            process_data(
                batch_data, memicos_source_set_id, file.replace(".json", "")
            )
